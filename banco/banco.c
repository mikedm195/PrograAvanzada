...
void atGen(){
    ...
    int lock = -1;
    cajero c;
    for(aux = generalesT;aux<(generalesT+E);aux++){        
        llega = (rand()%17)+5;
        ...
        while(lock){
            ...
            for(;i<5;i++){
                if(pthread_mutex_trylock((Gen+i))==0){
                    ...
                    c.num = i;
                    c.id = iniGen;
                    pthread_create(aux,NULL,operaG, &c);
                }
            }
        }
    }
...
int main(){
    ...
    pthread_create(atiendeGen,NULL,atGen,NULL);
    pthread_create(atiendeEmp,NULL,atEmp,NULL);
    /*
    int id = 1;
